                            ...
                            <table id="filteredAssetTable" class="table table-bordered table-hover display">
                                ...
                                <tbody>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach($position_status_tables as $position_status_table)
                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        @foreach ($item_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\Item::find($position_status_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach
                                        ...
                                        @foreach ($manufacturer_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\Manufacturer::find($position_status_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach

                                            <td>{{ $position_status_table[reset($serial_number_column)] }}</td>

                                        @foreach ($configuration_status_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\ConfigurationStatus::find($position_status_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach
                                        @foreach ($location_column as $columns)
                                            @php
                                                $locationId = $position_status_table[$columns];
                                            ...
                                            <td onclick="toggleLocationPopup('{{ $location->latitude }}', '{{ $location->longitude }}', '{{ $location->name }}', '{{ $location->address }}')">
                                                ...
                                                @php
                                                    $item = \App\Models\Location::find($position_status_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach

                                            <td>{{ $position_status_table[reset($description_column)] }}</td>

                                        {{-- @foreach ($description_column as $columns)
                                            <td>
                                                {{-- @php
                                                    $item = \App\Models\Item::find($position_status_table[$column]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }} 
                                            </td>
                                        @endforeach --}}
                                        ...
                                        @foreach ($position_status_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\PositionStatus::find($position_status_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach

                                            <td>{{ $position_status_table[reset($created_date_column)] }}</td>

                                            <td>
                                                <a href="{{ route('data-tables.filter-results.item.show', $position_status_table->id) }}" class="btn btn-primary btn-sm text-white mr-1"><i class="fa-solid fa-eye mr-2"></i>Show</a>
                                            </td>    
                                        ...
                                        {{-- @foreach ($visible_new_columns as $column)
                                            <td>{{ $position_status_table[$column] }}</td>
                                        @endforeach --}}
                                        {{-- <td>{{ $position_status_table[reset($visible_new_columns)] }}</td> --}}
                                        {{-- <td onclick="toggleLocationPopup('{{ $position_status_table->location->latitude }}', '{{ $position_status_table->location->longitude }}', '{{ $position_status_table->location->name }}', '{{ $position_status_table->location->address }}')">
                                            <div id="locationPopup">
                                                <div id="map" style="height: 25vh; width: 50vh;"></div>
                                            </div>
                                            {{ $position_status_table->location->name }}
                                        </td> --}}
                                    </tr>
                                    @endforeach
                                </tbody>
