                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach ($item_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\Item::find($configuration_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach
                                        ...
                                        @foreach ($manufacturer_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\Manufacturer::find($configuration_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach

                                            <td>{{ $configuration_table[reset($serial_number_column)] }}</td>

                                        @foreach ($configuration_status_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\ConfigurationStatus::find($configuration_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach
                                        @foreach ($location_column as $columns)
                                            @php
                                                $locationId = $configuration_table[$columns];
                                            ...
                                            <td onclick="toggleLocationPopup('{{ $location->latitude }}', '{{ $location->longitude }}', '{{ $location->name }}', '{{ $location->address }}')">
                                                ...
                                                @php
                                                    $item = \App\Models\Location::find($configuration_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach

                                            <td>{{ $configuration_table[reset($description_column)] }}</td>

                                        {{-- @foreach ($description_column as $columns)
                                            <td>
                                                {{-- @php
                                                    $item = \App\Models\Item::find($configuration_table[$column]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }} 
                                            </td>
                                        @endforeach --}}
                                        ...
                                        @foreach ($position_status_column as $columns)
                                            <td>
                                                @php
                                                    $item = \App\Models\PositionStatus::find($configuration_table[$columns]);
                                                @endphp
                                                {{ $item ? $item->name : '-' }}
                                            </td>
                                        @endforeach

                                            <td>{{ $configuration_table[reset($created_date_column)] }}</td>

                                            <td>
                                                <a href="{{ route('data-tables.filter-results.item.show', $configuration_table->id) }}" class="btn btn-primary btn-sm text-white mr-1"><i class="fa-solid fa-eye mr-2"></i>Show</a>
                                            </td>
                                        ...
                                        {{-- @foreach ($visible_new_columns as $column)
                                            <td>{{ $configuration_table[$column] }}</td>
                                        @endforeach --}}
                                        {{-- <td>{{ $configuration_table[reset($visible_new_columns)] }}</td> --}}
